...
transversal line, then alternate interior angles are equal. Note: We
proved the converse earlier \textit{without} use of the parallel
postulate.
...
\alpha$. Decorating our picture more,
...
\tkzMarkAngle[size=0.8cm,thin](H,G,C)
\tkzLabelAngle[pos = 0.4](H,G,C){$\beta$}
...
\tkzMarkAngle[size=0.7cm,thin](G,H,B)
\tkzLabelAngle[pos = 0.5](G,H,B){$\gamma$}
...
\tkzMarkAngle[size=1.2cm,thin](G,H,D)
\tkzLabelAngle[pos = 1](G,H,D){$\beta$}
...
\begin{enumerate}
\item Start by constructing lines $\l_1$ and $\l_2$ with $\l_1$
  perpendicular to $\l_2$ at point $A$. On $\l_1$ add point $B$ so
  that $|AB|=a$. Next construct $\l_3$ perpendicular to $\l_1$ through
...
\item Finally, add a segment to our figure and use a triangle congruence
...
\tkzMarkAngle[arc=||,size=.6cm,thin](B,A,C)
\tkzMarkAngle[arc=||,size=.6cm,thin](D,C,A)
