...
\begin{listOutcomes}
\item Define Euclidean geometry,
 \item Prove that in Euclidean geometry, the sum of the interior angles of a triangle is $180^\circ$.
 \item Show that Euclidean geometry has a Cartesian coordinate system.
\end{listOutcomes}
We saw in the last chapter that neutral geometry does not assume that two parallel lines must intersect a third line in such a way that the sum of the interior angles (ie, the two angles between the parallel lines) is the same as the sum of two right angles (ie, $180^\circ$).  A (very not obviously) equivalent statement is:

\begin{axiom}[E5]
Through a point not on a line there passes a unique parallel line.
\end{axiom}

Neutral geometry together with E5 is called \emph{Euclidean} or \emph{flat geometry}
(Euclidean geometry). Euclidean geometry is probably what you think about when you think about geometry. We will see later that there is another geometry called \textit{hyperbolic geometry} (\textbf{HG})
that satisfies all the postulates of neutral geometry but not E5. In it,
the sum of the interior angles of a triangle will \textit{always} be
less than $180^\circ$! 

In Problem 16, %\ref{altinteriorangles}, 
we will show that in Euclidean geometry,  two parallel lines must intersect a third line in such a way that the sum of the interior angles is $180^\circ$.
...
\end{hint}
\begin{hint}
 Watch the solution at \url{https://youtu.be/tMBADmI6LnY} or read the file at \url{https://osu.instructure.com/courses/84670/files/23572065/}. 
 
Your solution should make sense to someone who has not watched the video or read the notes file. In particular, you should explain how to draw the diagram and put the proof in your own words.
\end{hint}
\begin{freeResponse}
...
\begin{problem}\label{altinteriorangles}
 Show that if two parallel lines are cut by a transverse line, then the sum of the interior angles is $180^\circ$.
 
 
\begin{hint}
 Watch hint video at \url{https://youtu.be/7ttySwGJpZg} or read the notes at \url{https://osu.instructure.com/courses/84670/files/23572065/}.
\end{hint}
 
\begin{freeResponse}
 From the previous problem, if two parallel lines are cut by a transversal line, then alternate interior angles are equal labeled $\alpha$ in the diagram. Then the interior angles have measure $\alpha$ and $\beta$ as in the following diagram. Since the two angles form a straight line, $\alpha+\beta=180^\circ$.
...
\tkzMarkAngle[size=0.5cm,thin](A,H,G)
\tkzLabelAngle[pos = -0.3](A,H,G){$\alpha$}
...
parallel sides.
\end{hint}
%%%%%%%%%%%%%CHANGED FROM IN PERSON SEMESTERS%%%%%%%%%%%%%%%%%%%
\begin{proof}
...
established about neutral geometry to show that in Euclidean geometry the sum
...
\begin{problem}
Show that in Euclidean geometry the sum of the interior angles of a
...
Now we will show in Euclidean geometry that given any positive real numbers
$a$ and $b$, there exist rectangles with adjacent side of lengths $a$
and $b$. Your task is to fill-in the details of the proof below. The notes from when we talked about this problem in class are at \url{https://osu.instructure.com/files/23572263/} or you can rewatch the class recording \url{https://osu.zoom.us/rec/share/UH_Zr42lQKe2VYVa9t_AVoiPiKuvXj5rcA5M9h9y30-7oMjUSiSoxnFCwXIo8vs.U26znUPozxyOrGaT} (Access Passcode: Gq7\%m.jc) 
...
\begin{problem}
Consider the ordered pair $(x,y)\in\mathbb{R}^2$ to be the corner of rectangle with one vertex at the origin, side lengths $|x|$ and $|y|$, and the sign of $x$ and $y$ moving in either the positive or negative direction along the $x$ and $y$ axes. This is a \emph{Cartesian coordinate system}.
...
\begin{hint}
 For a longer explanation of what the question is asking and where the map comes from, watch the video at \url{https://youtu.be/ffal4JkqrEw}.
\end{hint}
\begin{proof}
...
\begin{listOutcomes}
 \item Use the axioms of Euclidean geometry to prove the Pythagorean theorem,
 \item Prove the Euclidean distance formula.
\end{listOutcomes}
...
do not exist in hyperbolic or spherical. (Try drawing two triangles
that are similar but not congruent on a perfectly spherical balloon!)

\begin{problem}
State and prove the Pythagorean theorem in Euclidean geometry. Do not use the distance formula. 
