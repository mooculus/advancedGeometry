...
\end{tikzpicture}
\end{image}
We should explain why the smaller quadrailateral is in fact a
square. First note that each side has the same length, as by SAS the
sides are made of conrguent triangles. Second note that each of the
...
and $(b_2-b_1)$ whose hypotenuse is the shortest path between the two points. By the Pythagorean theorem, the length of this path is given by 
\[
\sqrt{(a_{2}-a_{1})^{2}+(b_{2}-b_{1})^{2}}.
\]
\end{freeResponse}
