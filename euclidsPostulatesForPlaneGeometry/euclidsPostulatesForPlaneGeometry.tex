\documentclass[noauthor,nooutcomes,12pt,hints,newpage,handout]{ximera}
...
fundamentals. In the \textit{Elements}, Euclid gives five axioms for
plane geometry, the first four of which seem to be `obvious'
reflections of physical reality. In paraphrased form, they are:
...
%%         \tkzMarkAngle[size=0.5cm,thin,arc=ll](E,D,B)
...
\begin{problem}\label{ng_parallel}
  Let's talk about alternate interior angles.
  \begin{enumerate}
  \item Show that in \textbf{neutral geometry}, if two lines cut by a
    transversal line have a pair of congruent alternate interior
    angles, then they are parallel.
  \item Explain why spherical geometry is not a neutral geometry.
  \end{enumerate}
\begin{hint}
For the first part, suppose the assertion is false for some pair of
lines. Find a triangle that violates the conclusion of the previous
problem.

For the second, draw a picture.
\end{hint}
\begin{freeResponse}
...
$180^\circ$.%%  In fact, there is an entire list of geometry facts that
%% are true if and only if the parallel postulate is true. You can find
%% some at:
%% \url{https://orion.math.iastate.edu/lhogben/classes/parallelpostulate.pdf}.
...
\tkzMarkAngle[size=1.5cm,thin,arc=||](C,A,B)
%\draw[step=.5cm] (0,0) grid (10,5);
\end{tikzpicture}
\end{image}
...
interior angles is $(180+x)^\circ$ but one of this triangle's angle's measures less than
\[
\frac{|\angle CAB|}{2^n},
\]
for any value of $n$.
