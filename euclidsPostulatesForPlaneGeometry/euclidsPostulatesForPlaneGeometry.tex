...
On the other hand, by our previous work%by Problem \ref{118} (b), 
...
(self-) congruent to the quadrilateral $DCBA$. 
For the second implication in a) suppose that $\left\vert AB\right\vert >\left\vert CD\right\vert $. 
Construct $A^{\prime}$ on $\overline{AB}$ so that $\left\vert A^{\prime}B\right\vert
=\left\vert CD\right\vert $. By Problem \ref{18}%
\[
...
The proof of the third implication in a) is the same as the proof of the
second implication--just interchange $A$ and $D$ and interchange $B$ and $C$.
...
\angle BAD>\angle ADC.
\]
Contradiction! Etc., etc.\pagebreak
\end{problem}
