...
\begin{freeResponse}
In this section we started working in \dfn{$\boldsymbol{K}$-warped space}, noted by $(x, y, z)$ coordinates. We used the following rule to transition between euclidean and  $(x, y, z)$ coordinates:
\begin{align*}
\hat{x}  &  =x\\
\hat{y}  &  =y\\
\hat{z}  &  =Rz.
\end{align*}
In \dfn{$\boldsymbol{K}$-warped space} the following surface,
\[
K\left(x^{2}+y^{2}\right)+z^{2}=1,
\]
 generates the hyperbolic plane when $K<0$, the euclidean plane when $K=0$, and the sphere of radius $R$ when $K> 0$.
We computed the euclidean dot product in $(x, y, z)$ coordinates and defined it as the \textbf{$\boldsymbol{K}$-dot-product} such that,
\[
V_{1}\bullet_{K}V_{2}  = V_{1} 
\begin{bmatrix}
1 & 0 & 0\\
...
V_{2}^\transpose.
\]
Lastly we calculated formulas for length, area, and the angle between vectors in $(x,y,z)$-coordinates in terms of the  \textbf{$\boldsymbol{K}$-dot-product}.
\end{freeResponse}
\end{problem}
