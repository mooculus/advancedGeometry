...
Based on the following result, we say that ``Cross-ratio is invariant under stereographic
projection.'' This statement will have more meaning 
...
\coordinate (D) at ({2*cos(338)},{2*sin(338)});

\coordinate (A') at (-3.21,-3);
\coordinate (B') at (-.59,-3);
\coordinate (C') at (2.31,-3);
\coordinate (D') at (4.47,-3);
