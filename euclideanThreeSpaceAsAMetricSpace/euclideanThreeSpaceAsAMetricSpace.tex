...
%In this book, we will study plane, spherical and hyperbolic geometry in \textit{two}-dimensions. Each one of these geometries looks the same at each of its points and it also looks  the same in every direction based at any of its points. But to study them all at the same time and in a uniform way we will need to visualize them all as different surfaces lying in some common \textit{three}-dimensional space. Let's examine this ``ambient'' space.
...
%% {a}_{2},\hat{b}_{2},\hat{c}_{2}\right)  $ based at the same point in
