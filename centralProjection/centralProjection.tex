...
projection:
\begin{center}
  \scalebox{.6}{\begin{tikzpicture}[geometryDiagrams]  
      %% x-axis
...
\begin{tikzcd}[column sep=large,row sep = huge,ampersand replacement = \&]
  (x_c,y_c)\in \R^2 \ar[r,|->,"K_c"] \& (x, y ,z) \in \R^3
\end{tikzcd}
\end{center}
...
\begin{problem}
  ...
  \begin{center}
    \begin{tikzcd}[column sep=large,row sep = huge,ampersand replacement=\&]
      t\in\R \ar[r,|->,"\gamma_\mathrm{c}"] \ar[rd,|->,swap,"\gamma=K_c\circ \gamma_\mathrm{c}"] \&  (x_c(t),y_c(t))\in\R^2\ar[d,|->,"{K_c}"] \\
