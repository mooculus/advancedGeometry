\documentclass[12pt]{ximera}

\input{../preamble}

\begin{document}
\begin{abstract}
\end{abstract}


\begin{tikzpicture}[geometryDiagrams]  
      %% x-axis
      \draw [thin,->] (3,1.5)--(-3,-1.5); 
      \node[below left] at (-3,-1.5) {$x$};

      %% y-axis
      \draw [thin,->] (-3.5,0)--(3.5,0); 
      \node[right] at (3.5,0) {$y$};

      %% z-axis
      \draw [thin,->] (0,-2.5)--(0,3.5); 
      \node[above] at (0,3.5) {$z$};

      %% NP
      \draw [fill=black] (0,1.75) circle (.075);
      
      %% plane at z=1
      \draw (-5,.75)--(-1,2.75)--(5,2.75)--(1,.75)--cycle;
      \node[right] at (4.2,2.25) {$z=1$};
      
      %% sphere
      \draw[dashed,thin] (1.45,0.98) arc (34:155:1.75);
      \draw (-1.58,0.75) arc (155:394:1.75);
      %%%% Equator
      \draw (-1.75,0) arc (180:360:1.75 and .5);
      \draw[dashed,thin] (1.75,0) arc (0:180:1.75 and .5);

      \node[right] at (1,-1.5) {$K(x^2+y^2) + z^2 = 1$};
      \node[right] at (1,-2) {$K>0$};

      
      %% point on plane and point on sphere connected by central line
      %%%% point on plane
      \draw[fill=black] (-1.6,2.2) circle (.075);
      \node[right] at (-1.6,2.2) {$(x_c,y_c)$};
      %%%% line to point
      \draw[thin] (0,0)--(-1.6,2.2); 
      %%%% point on sphere
      \draw[fill=black] (-.88,1.21) circle (.075);
      \node[left] at (-1.3,1.21) {$(x,y,z)$};


      

      
\end{tikzpicture}  



\begin{tikzpicture}[geometryDiagrams]  
      %% x-axis
      \draw[thin,->] (3,1.5)--(-3,-1.5); 
      \node[below left] at (-3,-1.5) {$x$};

      %% y-axis
      \draw[thin,->] (-3.5,0)--(3.5,0); 
      \node[right] at (3.5,0) {$y$};

      %% z-axis
      \draw[thin,->] (0,-2)--(0,3.5); 
      \node[above] at (0,3.5) {$z$};

      %% NP
      \draw[fill=black] (0,1.75) circle (.075);

      
      %% plane at z=1
      \draw (-5,.75)--(-1,2.75)--(5,2.75)--(1,.75)--(-5,.75)--cycle;
      \node[right] at (4.2,2.25) {$z=1$};
\end{tikzpicture}  





\begin{tikzpicture}[geometryDiagrams]  
      %% x-axis
      \draw[thin,->] (3,1.5)--(-3,-1.5); 
      \node[below left] at (-3,-1.5) {$x$};

      %% y-axis
      \draw[thin,->] (-3.5,0)--(3.5,0); 
      \node[right] at (3.5,0) {$y$};

      %% z-axis
      \draw[thin,->] (0,-1)--(0,3.5); 
      \node[above] at (0,3.5) {$z$};

      %% NP
      \draw[fill=black] (0,1.75) circle (.075);
      
      %% plane at z=1
      \draw (2.12,2.75)--(5,2.75)--(1,.75)--(-5,.75)--(-1.67,2.42);
      \draw[dashed,thin] (-1.67,2.42)--(-1,2.75)--(2.12,2.75);
      \node[right] at (4.2,2.25) {$z=1$};


      %% 2-sheeted hyperboloid
      \draw[domain=-3.5:3.5] plot (\x,{sqrt(1.75^2+\x*\x)});
      %%%% Equator
      \draw (3.5,4) arc (0:360:3.5 and .8);
      \node[right] at (3.5,4) {$K(x^2+y^2) + z^2 = 1$};
      \node[right] at (3.5,3.5) {$K<0$};

      %% point on plane and point on hyperboloid connected by central line
      %%%% point on plane
      \draw[fill=black] (-2.18,3) circle (.075);
      \node[right] at (-2.18,3) {$(x_c,y_c)$};
      %%%% line to point
      \draw[thin] (0,0)--(-2.18,3); 
      %%%% point on sphere
      \draw[fill=black] (-.88,1.21) circle (.075);
      \node[left] at (-.88,1.21) {$(x,y,z)$};
      
\end{tikzpicture}  







For lines in spherical geometry:


\begin{tikzpicture}[geometryDiagrams,scale=2]  
      %% x-axis
      \draw [thin,->] (2.5,1.25)--(-2.5,-1.25); 
      \node[below left] at (-2.5,-1.25) {$x$};

      %% y-axis
      \draw [thin,->] (-2.5,0)--(3,0); 
      \node[right] at (3,0) {$y$};

      %% z-axis
      \draw [thin,->] (0,-2)--(0,2.5); 
      \node[above] at (0,2.5) {$z$};

      
      
      %% sphere
      \draw (1.75,0) arc (0:360:1.75);
      %%%% Equator
      \draw (-1.75,0) arc (180:360:1.75 and .5);
      \draw[thin] (1.75,0) arc (0:180:1.75 and .5);

      
      
      %%%% line to point
      \draw[thin,dashed] (0,0)--(.88,1.21);
      \draw[thin,dashed] (0,0)--(.88,-.2)--(.88,1.21); 
      %%%% point on sphere
      \draw[fill=black] (.88,1.21) circle (.075);
      \node at (1.8,1.6) {$(x,y,z)$};

      %% angles:
      \draw[thin,->] (-.2,-.1) arc (206:340:.35 and .1);

      \draw[thin,->] (0,.8) arc (90:54:.8);



      

      
\end{tikzpicture}  





\begin{tikzpicture}
    \begin{axis}[tick label style={font=\scriptsize},axis on top,
    axis lines=center,
    view={110}{25},
    name=myplot,
    xtick=\empty,
        ytick=\empty,
        ztick=\empty,
    ymin=-.1,ymax=1.2,
    xmin=-.1,xmax=1.2,
    zmin=-.2, zmax=2.1,
    every axis x label/.style={at={(axis cs:\pgfkeysvalueof{/pgfplots/xmax},0,0)},xshift=-1pt,yshift=-4pt},
    xlabel={\scriptsize $x$},
    every axis y label/.style={at={(axis cs:0,\pgfkeysvalueof{/pgfplots/ymax},0)},xshift=5pt,yshift=-3pt},
    ylabel={\scriptsize $y$},
    every axis z label/.style={at={(axis cs:0,0,\pgfkeysvalueof{/pgfplots/zmax})},xshift=0pt,yshift=4pt},
    zlabel={\scriptsize $z$},
        colormap/cool,
      ]
      \addplot3[gray,->,domain=0:45,samples y=0] ({.3*cos(x)},{.3*sin(x)},0); %% angle for theta
      \addplot3[gray,->,domain=0:27,samples y=0] ({.3*cos(45)*sin(x)},{.3*sin(45)*sin(x)},{.3*cos(x)}); %% angle for phi
      \addplot3[gray,domain=0:2.1,samples y=0,->] ({x*cos(45)*sin(27)},{x*sin(45)*sin(27)},{x*cos(27)}); %% line for rho
       
      \addplot3[gray,domain=0:1,samples y=0,dashed] ({x*cos(45)},{x*sin(45)},0); %% line for theta
      \addplot3[gray,domain=0:2,samples y=0,dashed] ({1*cos(45)},{1*sin(45)},x); %% line for z
      \node at (axis cs:{.5*cos(22.5)},{.5*sin(22.5)},0) {$\theta$};
      \node[above] at (axis cs:{.3*cos(45)*sin(17)},{.3*sin(45)*sin(17)},{.3*cos(17)}) {$\varphi$};
       
      \node[above] at (axis cs:{1*cos(45)*sin(27)},{1*sin(45)*sin(27)},{1*cos(27)}) {$r$};
       
      \filldraw [black] (axis cs:{1*cos(45)},{1*sin(45)},1.95) circle (2.5pt);       
      \node[right] at (axis cs:{1*cos(45)},{1*sin(45)},2) {$(x,y,z)$};
    \end{axis}
  \end{tikzpicture}

\end{document}
