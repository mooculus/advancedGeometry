...
real number $r$, we define a mapping with center $\left(
...
\begin{problem}
Using cartesian coordinates for the plane, show that the mapping $\mathrm{mag}(x,y)$
...
\begin{freeResponse}
Since an angles rays lie on lines, and dilations map lines to lines parallel to themselves, and we know from a previous
packet that angles with parallel rays are equal,  dilations of the plane preserve
angles.
\end{freeResponse}
\end{problem}
...
\begin{hint}
Express a previous answer into the form of a magnification.
\end{hint}
\begin{freeResponse}
Write
\begin{align*}
