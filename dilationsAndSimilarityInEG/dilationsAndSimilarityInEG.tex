\documentclass[hints,handout]{ximera}
...
\section{Dilations in Euclidean Geometry}
\begin{listOutcomes}
 \item Show how dilations affect vectors.
 \item Show how dilations affect lines.
 \item Show how dilations affect angles.
\end{listOutcomes}

We start this section off by asking you to prove a general result.

\begin{problem}
  Let $X$ be a set and $f: X \to X$. Prove that a function $f^{-1}$ exists if and
  only if $f$ is one-to-one and onto.
\begin{hint}
 Remember the definition of a function means that $f^{-1}$ is a function if and only if for every input $x$ in the domain $X$, there exists a unique output $y$ in the range (in this case, also $X$).
\end{hint}
  \begin{freeResponse}
...
\begin{definition}
 A \emph{transformation} or \emph{mapping} is a function $f$ that takes a set $X$ to itself. That is $f:X\to X$. Mathematicians often use function, map, and transformation interchangeably. 
\end{definition}
\begin{definition}
...
Dilations are like inflation and deflation or zooming in or out on an image. Inflation or zooming in does not introduce gaps (onto) or folds (one-to-one). One point stays the same when zooming in and lines through the center are stretched.
...
\end{hint}
\begin{hint}
 See video solution at \url{https://youtu.be/ZkNg-dp2ijQ} or notes \url{https://osu.instructure.com/courses/84670/files/23562177/}. The video and notes give a proof that $D$ is one-to-one and onto without using the result of the previous problem. 
\end{hint}
\begin{proof}We need to verify that $D$ satisfies the four parts of the definition of a dilation.
\begin{description} 
\item[(a)] By the previous problem, we can prove that $D$ is one-to-one and onto by finding its inverse. To do this, let $(x,y),(z,w)\in\mathbb{R}^2$ such that 
\begin{equation}
(z,w):= D\left(  x,y\right)  =\left(  x_{0},y_{0}\right)  +r\left(  x-x_{0},y-y_{0}\right).
\end{equation}
Since $x,y,x_0,y_0,r\in\mathbb{R}$, we know such a $(z,w)$ exists (This is the proof of onto!).  Now we solve for $(x,y)$.
\begin{align}
(z,w)&=\left(  x_{0},y_{0}\right)  +r\left(  x-x_{0},y-y_{0}\right)\\
(z-x_0,w-y_0)&=r(x-x_0,y-y_0)\\
\left(\frac{z-x_0}{r},\frac{w-y_0}{r}\right)&=(x-x_0,y-y_0)\\
\left(\frac{z-x_0}{r}+x_0,\frac{w-y_0}{r}+y_0\right)&=(x,y).
\end{align}
Thus, $D^{-1}(z,w):=\left(\frac{z-x_0}{r}+x_0,\frac{w-y_0}{r}+y_0\right)$. Since we found an inverse function, $D$ must be one-to-one and onto.
\item[(b)] We must show that the center of the dilation is fixed, that is we must show that 
\[
D(x_0,y_0) = (x_0,y_0).
\]
Write
...
&=(x_0,y_0).
\end{align*}
\item[(c)] Next we must show that every line through the fixed point goes to
...
\item[(d)] Finally, we must show that all distances are multiplied by the
...
\begin{hint}
Solve for $(x,y)$ in terms of $(\underline{x},\underline{y})$. Look for where this is done in the previous problem.
\end{hint}
\begin{freeResponse}
Write
\begin{align*}
...
\begin{problem}
Show that a dilation by a factor of $r$ takes any vector to $r$ times itself.
\begin{hint}
Write a vector $\vec{u}$ as $(a,b)$, where the head of $\vec{u}$ is on $(a,b)$ when the tail is on $(0,0)$. That is, $\vec{u}$ has slope $\frac{b}{a}$ and length $\sqrt{a^2+b^2}$. 

Look for where this was done in a previous problem.
\end{hint}
\begin{freeResponse}
...
\begin{hint}
Use the parametric formula for a line.  For a longer review of parameterizing lines using vectors, see \url{https://youtu.be/ZwgeCXuIROA} and \url{https://youtu.be/ZkNg-dp2ijQ?t=532} at 8:52.
\end{hint}
...
factor of $r^{2}$.
\begin{hint}
Break the region into rectangles. Focus on explaining why this fact is true for rectangles, then why it makes sense to be true in general. 
\end{hint}
\begin{freeResponse}
...
\section{Similarity in {Euclidean Geometry}}
\begin{listOutcomes}
 \item Prove two triangles are similar if and only if the corresponding sides are proportionally with the same constant of proportionality.
 \item Prove two triangles are similar if and only if the corresponding angles are congruent.
 \item Prove two triangles are similar if the corresponding sides are parallel or perpendicular.
\end{listOutcomes}
\begin{definition}
...
\end{enumerate}
\begin{hint}
For the first part, you have to start from the hypothesis that the
two triangles satisfy our definition of similar triangles.
\end{hint}
\begin{hint}
For the second part, you have to start from the assumption that
...
$\triangle A'B'C'$.
\end{hint}
%\begin{freeResponse}%%%%%%%%%%%%%CHANGE FROM IN PERSON%%%%%%
\begin{proof}
...
\begin{hint}
For the first part, you have to start from the assumption that the
two triangles satisfy our definition of similar triangles.
\end{hint}
\begin{hint}
For the second part, you have to start from the assumption that
...
\end{freeResponse}%%%%%%%%%%%%%%CHANGE FROM IN PERSON%%%%%%%
\begin{proof}
